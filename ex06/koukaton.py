...
class Shot:
    def __init__(self, chr: Bird):
        self.sfc = pg.image.load("fig/ball1-1.png")
...
def main():
    ...
    bkd2 = Bomb2((255,0,0), 10, (-2.5, +1), scr)
...
#レベル1
        if key_states[pg.K_1] == True:
            beams = []
            pg.time.set_timer(35, 5000)
            pg.time.set_timer(36, 6000)
            while True:
                scr.blit()
                font = pg.font.Font(None,70)
                txt = font.render(f"{kill}/10", True, "YELLOW")
...
#レベル2
        if key_states[pg.K_2] == True:
            ...
            while True:
                scr.blit()
                font = pg.font.Font(None,70)
                txt = font.render(f"{kill}/20", True, "YELLOW")
                screen.blit(txt,[100,800])
                for event in pg.event.get():
                    if event.type == pg.QUIT:
                        return
                ...
                if len(beams) != 0:
                    for beam in beams:
                        ...
                        if bkd.rct.colliderect(beam.rct):
                            ...
                            kill += 1
                            counter += 1
                            bkd = Bomb((255,0,0), 10, (-2.5, +1), scr)
                        if bkd2.rct.colliderect(beam.rct):
                            del bkd2
                            kill += 1
                            counter += 1
                            bkd2 = Bomb2((255,0,0), 10, (-3.5, +1), scr)
                if kkt.rct.colliderect(bkd.rct): #爆弾インスタンスのrect変数
                    tkm.showwarning("お前が殺した。","あなたが殺した。")
                    return
                if kkt.rct.colliderect(bkd2.rct): #爆弾インスタンスのrect変数
